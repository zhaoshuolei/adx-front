...
22 个页面均已建立可运行视图，不再包含占位路由。页面使用本地演示数据完成主要交互，
生产环境关闭 `VITE_USE_MOCK` 后切换到 Swagger 定义的真实接口。
...
当前仓库已完成设计系统、页面清单、导航骨架、主题系统和全部 22 个页面视图。
完整交付仍需在后端测试环境执行联调、权限矩阵核验和真实数据回归。
