  ...
  <div class="ad-space-page">
    ...
    <section class="adx-panel ad-space-filter">
      ...
      <div class="ad-space-filter__grid">
        ...
        <label class="adx-field"><span class="adx-field__label">媒体应用</span><span class="adx-input-shell"><select v-model.number="filters.mediaId"><option :value="0">全部媒体</option><option v-for="media in mediaOptions" :key="media.id" :value="media.id">{{ media.appName }}</option></select></span></label>
        <label class="adx-field"><span class="adx-field__label">启用状态</span><span class="adx-input-shell"><select v-model="filters.status"><option value="">全部状态</option><option value="1">已启用</option><option value="0">已停用</option></select></span></label>
        <label class="adx-field"><span class="adx-field__label">广告状态</span><span class="adx-input-shell"><select v-model="filters.adStatus"><option value="">全部广告状态</option><option value="PRODUCTION">正式投放</option><option value="TEST">测试</option></select></span></label>
    ...
    <section class="adx-panel ad-space-table-panel">
      ...
      <footer v-if="result.totalPages > 1" class="ad-space-pagination"><span>共 {{ result.totalElements }} 条</span><div><button type="button" :disabled="result.page === 0" @click="load(result.page - 1)">上一页</button><button v-for="page in paginationPages" :key="page" type="button" :class="{ active: result.page === page }" @click="load(page)">{{ page + 1 }}</button><button type="button" :disabled="result.page >= result.totalPages - 1" @click="load(result.page + 1)">下一页</button></div><select v-model.number="pageSize" @change="load(0)"><option :value="10">10 条/页</option><option :value="20">20 条/页</option><option :value="50">50 条/页</option></select></footer>
    </section>
