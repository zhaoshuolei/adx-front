...
<template>
  <div class="config-page">
    ...
    <section class="adx-panel config-filter"><label class="adx-input-shell config-search"><Search class="adx-icon" /><input v-model="keyword" placeholder="配置键或说明" /></label><span class="adx-input-shell adx-select-shell"><select v-model="scope"><option value="">全部消费方</option><option value="engine">Engine</option><option value="admin">Admin</option><option value="shared">Shared</option></select></span><span class="adx-input-shell adx-select-shell"><select v-model="category"><option value="">全部生效方式</option><option value="immediate">热更新</option><option value="restart">重启生效</option></select></span><button class="adx-button adx-button--secondary" type="button" @click="keyword = ''; scope = ''; category = ''">重置</button></section>
