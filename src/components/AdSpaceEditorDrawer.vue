        ...
        <div class="adx-drawer__body">
          <form id="adSpaceForm" class="ad-space-form" @submit.prevent="submit">
            <div class="ad-space-form__section"><h3>基础信息</h3><div class="ad-space-form__grid">
              <label class="adx-field"><span class="adx-field__label">媒体应用 *</span><span class="adx-input-shell adx-select-shell"><select v-model.number="form.mediaId" :disabled="mode === 'edit'"><option :value="0">请选择</option><option v-for="media in mediaOptions" :key="media.id" :value="media.id">{{ media.appName }}</option></select></span><small v-if="errors.mediaId" class="ad-space-form__error">{{ errors.mediaId }}</small></label>
              <label class="adx-field"><span class="adx-field__label">广告类型 *</span><span class="adx-input-shell adx-select-shell"><select v-model="form.adType"><option value="BANNER">横幅</option><option value="VIDEO">激励视频</option><option value="NATIVE">原生</option><option value="SPLASH">开屏</option><option value="INTERSTITIAL">插屏</option></select></span></label>
            ...
            <div class="ad-space-form__section"><h3>投放配置</h3><div class="ad-space-form__grid">
              <label class="adx-field"><span class="adx-field__label">广告状态</span><span class="adx-input-shell adx-select-shell"><select v-model="form.adStatus"><option value="PRODUCTION">正式投放</option><option value="TEST">测试</option></select></span></label>
              <label class="adx-field"><span class="adx-field__label">广告尺寸</span><span class="adx-input-shell adx-select-shell"><select v-model="form.adSizeId"><option :value="undefined">不限制</option><option v-for="size in adSizeOptions" :key="size.id" :value="size.id">{{ size.name }} · {{ size.width }}×{{ size.height }}</option></select></span></label>
              ...
              <label class="adx-field ad-space-form__full"><span class="adx-field__label">分组策略方案</span><span class="adx-input-shell adx-select-shell"><select v-model="form.strategyProfileId"><option :value="undefined">不绑定策略方案</option><option v-for="strategy in strategyOptions" :key="strategy.id" :value="strategy.id">{{ strategy.name }} · {{ strategy.code }}</option></select></span></label>
